...
public:
     ...
     */
    BabySitter(const ros::NodeHandle &nh, BagLogger *logger,
            const std::string &node_name, const std::string &topic, int warn_ms,
            int error_ms, int warn_count_thresh = 5, int timeout_ms = 2000,
...
template<class M>
BabySitter<M>::BabySitter(const ros::NodeHandle &nh, BagLogger *logger,
        const std::string &node_name, const std::string &topic, int warn_ms,
        int error_ms, int warn_count_thresh, int timeout_ms, int start_delay_ms)
{
...
{
    ROS_INFO_STREAM(
            NODE_FUNC << node_name_ << ", node state:" << parseNodeState(node_state_) << ", device state: " << parseDeviceState(device_state_));
}
...
void BabySitter<M>::checkNodeState()
{
    ...
    default:
        ROS_WARN_STREAM_THROTTLE(10,
                NODE_FUNC << node_name_ << ": unknown node state: " << parseNodeState(node_state_));
        break;
    }
}
...
{
    ROS_INFO_STREAM(
            NODE_FUNC << node_name_ << ": changing state from: " << parseNodeState(node_state_) << " to: " << parseNodeState(node_state));
...
void BabySitter<M>::deviceCB(const ros::MessageEvent<M const> &event)
{
    ...
    if (now_ms - start_time_ms_ < start_delay_ms_)
    {
        ROS_WARN_STREAM_THROTTLE(1.0,
                NODE_FUNC << node_name_ << ":message received during start delay");
    }
    ...
    if (latency_ms >= error_ms_)
    {
        ROS_ERROR_STREAM_THROTTLE(1.0,
                NODE_FUNC << node_name_ << ": max latency error: " << latency_ms << "(" << error_ms_ << ")");
        device_state_ = DeviceState::ERROR;
    } else if (latency_ms >= warn_ms_)
    {
        ROS_WARN_STREAM_THROTTLE(1.0,
                NODE_FUNC << node_name_ << ": latency warning: " << latency_ms << "(" << warn_ms_ << ")");
        device_state_ = DeviceState::WARN;
        warn_count_++;
        if (warn_count_ >= warn_count_thresh_)
        {
            ROS_ERROR_STREAM_THROTTLE(1.0,
                    NODE_FUNC << node_name_ << ": count latency error: " << warn_count_ << "(" << warn_count_thresh_ << ")");
            device_state_ = DeviceState::ERROR;
        }
    } else
    {
        if (device_state_ != DeviceState::OK)
        {
            ROS_INFO_STREAM_THROTTLE(1.0,
                    NODE_FUNC << node_name_ << ": latency ok: " << latency_ms);
...
long BabySitter<M>::nowMS()
{
    ...
    long now_ms = (long) (now.nsec / NSECS_IN_MSECS)
            + (long) now.sec * MSECS_IN_SECS;
    return now_ms;
}
...
void BabySitter<M>::heartbeatCB(const ros::TimerEvent &event)
{
    ...
    ROS_INFO_STREAM_THROTTLE(LOG_PERIOD_S,
            NODE_FUNC << node_name_ << " node:" << parseNodeState(node_state_) << ", state: " << parseDeviceState(device_state_) << ", max:" << max_ms_ << ", min: " << min_ms_ << ", ave: " << ave_ms_ << ", freq: " << freq_hz_);
}
